/**
 * Silent token refresh (Next 16 proxy — middleware successor).
 *
...
export default async function proxy(request: NextRequest): Promise<NextResponse> {
  ...
  if (!refresh) return NextResponse.next(); // can't refresh; verifySession will gate
  ...
  if (!issuer) return NextResponse.next();
  ...
  try {
    ...
    if (!j.id_token) return NextResponse.next();
...
export const config = {
  matcher: [
    {
      // App + API, excluding static assets, the auth-flow routes (they own their
      // cookies), and the public unsubscribe surface.
      source: "/((?!_next/static|_next/image|favicon.ico|auth/callback|auth/start|auth/logout|unsubscribe|api/unsubscribe).*)",
      missing: [
