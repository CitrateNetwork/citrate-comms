...
|---|---|---|
| [`RelayCommitOrder.tla`](RelayCommitOrder.tla) | `comms-relay/src/lib.rs::submit` (`accepted_commit` + epoch advance) and `comms-core/src/mls.rs` (epoch +1 per Commit) | `NoEpochFork`, `MonotoneEpoch`, `AppliedExists`, `Convergence` (liveness) |
